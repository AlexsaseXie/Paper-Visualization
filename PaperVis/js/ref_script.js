...
function updateText() {
    ...
    for (let year = 2001; year < 2019; year++) {
        if (paperData.hasOwnProperty(year.toString())) {
            ...
            let citationScale = d3.scaleLinear()
    ...
    let pointList, lineGeneratorBasis;
    for (let i = 0; i < referencePaperPos.length; i++) {
        pointList = [];
        pointList.push({x: (selectedPaperPos.x + Math.min(displayTitleLength, selectedPaper.title.length)), y: selectedPaperPos.y});
        pointList.push({x: (selectedPaperPos.x + Math.min(displayTitleLength, selectedPaper.title.length) + 10), y: selectedPaperPos.y});
        pointList.push({x: (referencePaperPos[i].x - 10), y: referencePaperPos[i].y});
        pointList.push(referencePaperPos[i]);

        lineGeneratorBasis = d3.line()
            .x(d => d.x)
            .y(d => d.y)
            .curve(d3.curveBasis);
            // .interpolate('basis');
        ...
        svgGroup.append('path')
            ...
            .attr('d', lineGeneratorBasis(pointList));
    }

    for (i = 0; i < citedPaperPos.length; i++) {
        pointList = [];
        pointList.push({x: (citedPaperPos[i].x + Math.min(displayTitleLength, citedPaperPos[i].l)), y: citedPaperPos[i].y});
        pointList.push({x: (citedPaperPos[i].x + Math.min(displayTitleLength, citedPaperPos[i].l) + 10), y: citedPaperPos[i].y});
        pointList.push({x: (selectedPaperPos.x - 10), y: selectedPaperPos.y});
        pointList.push(selectedPaperPos);
...
function updatePaperDetails(paperDetail, year) {
    ...
    if (paperDetail) {
        ...
        div.html(
            ...
            "<p  align=\"left\">" + "Year: "+ year + span20 + "Cited by " +paperDetail.cited_count + "</p>" +
...
function updatePaperDetail(paperId) {
    ...
    updatePaperDetails(allPaperData[year][id], year);
}
...
function yearBar() {
    ...
    let dScale = d3.scaleOrdinal()
        .domain(dateList)
        .range(dateRange);
    let dAxis = d3.axisBottom(dScale);
    ...
    for (let year = 2001; year < 2019; year += 2) {
        ...
        selection.enter().append("text")
            ...
            .on("click", function () {
                ...
                selectedPaper = d3.select(this)._groups[0][0].__data__;
                let year = selectedPaper.year;
                let id = parseInt(selectedPaper.id.toString().substr(4, 3)).toString();

                let node_id = "paper-" + year + "-" + id;

                // 调用力导向图
                click_update(selectedPaper.title, node_id);

                references = d3.select(this)._groups[0][0].__data__.references;
