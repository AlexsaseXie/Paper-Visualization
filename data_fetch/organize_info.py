...
def get_title_keywords(raw_title):
    return_str = raw_title.strip(' .').lower()
    return_str = re.sub(r'[^\w\s]','', return_str)
    return_str_list = return_str.split(' ')

    keywords = set(filter(lambda x: len(x)>= 3, return_str_list)) 
    return keywords

def is_lists_title(title_list, title):
    for title_index,title_a in enumerate(title_list):
        not_contained_keyword = 0
        for t_k in title:
            if not t_k in title_a:
                not_contained_keyword += 1
                if not_contained_keyword >= 2:
                    break
...
def judge_cvpr(p_all):
    cvpr_matchers = [r'co([a-z.]*) vi([a-z.]*) pa([a-z.]*) re([a-z.]*)',r'co([a-z.]*) vi([a-z.]*) (and) pa([a-z.]*) re([a-z.]*)', r'cvpr']
...
def judge_iccv(p_all):
    iccv_matchers = [r'in([a-z.]*) co([a-z.]*) co([a-z.]*) vi([a-z.]*)',r'in([a-z.]*) co([a-z.]*) on co([a-z.]*) vi([a-z.]*)', r'iccv']
...
def main():
    ...
    for y in year:
        ...
        title_keyword_list = [get_title_keywords(p['title']) for p in y_papers]
        title_keyword_sets[y] = title_keyword_list
        ...
        for p_index,p_info in enumerate(y_info):
            ...
            for p_ref in p_info['reference_list']:
                if p_ref['conference']!='book':
                    ...
                    p_ref_title_origin = p_ref['title']
                    ...
                    if is_iccv:
                        ...
                        if not founded and len(probable_years) >= 1: 
                            p_ref_title_keyword = get_title_keywords(p_ref_title_origin)
                            if len(p_ref_title_keyword) >= 3:
                                for probable_year in probable_years:
                                    if probable_year in year:
                                        index_number = is_lists_title(title_keyword_sets[probable_year], p_ref_title_keyword)
                                        if index_number:
                                            new_reference_list.append({'year': probable_year, 'id': index_number})
                                            founded_f.write('KEYCONF!!! %s \ntitle: %s year: %s span:(%d,%d) \n' % (p_ref['title'] + ',' + p_ref_conference, p_ref_title, probable_years, m.span()[0], m.span()[1]))
                                            founded = True
                                            #print('founded in keywords set')
                                            break
                            if not founded:
                    ...
                    if is_iccv:
                        ...
                        for i in range( len(p_ref_split_list)-1, -1, -1):
                            tmp = p_ref_split_list[i].lower().strip()
                            if len(p_ref_split_list[i]) >= 8 and (not 'proc' in tmp) and (not 'ieee' in tmp) and (not 'in int' in tmp) and (not 'in computer vision' in tmp):
                                p_ref_title_origin = p_ref_split_list[i]
                                p_ref_title = get_title_string(p_ref_title_origin)
                                break
                        ...
                        for probable_year in probable_years:
                            ...
                            if (probable_year in year) and (p_ref_title in title_sets[probable_year]):
                                index_number = title_sets[probable_year][p_ref_title] 
                                new_reference_list.append({'year': probable_year, 'id': index_number})
                                founded_f.write('BOOK!!! %s \ntitle: %s year: %s span:(%d,%d) \n' % (p_ref['title'] , p_ref_title, probable_years, m.span()[0], m.span()[1]))
                                founded = True
                                break
